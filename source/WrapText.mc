...
class WrapText {
  ...
  hidden var SCREEN_SHAPE_SEMI_OCTAGON = 4;
  ...
  function initialize() {
    ...
    if (partNumber.equals("006-B3888-00") || partNumber.equals("006-B4005-00") || partNumber.equals("006-B3889-00")) {
      screenShape = SCREEN_SHAPE_SEMI_OCTAGON;
    }
    offset = 0;
  }
  ...
  function writeLines(dc, text, font, posY) {
    ...
    if( (screenShape == System.SCREEN_SHAPE_ROUND || screenShape == SCREEN_SHAPE_SEMI_OCTAGON) && posY < roundMargin ) {
      posY = roundMargin;
    }
    // On semi-octagons same
  ...
  function drawText( dc, width, posY, font, text ) {
    if (screenShape == SCREEN_SHAPE_SEMI_OCTAGON && posY < screenHeight / 2.5) {
      dc.drawText(screenWidth/2, posY, font, text, Gfx.TEXT_JUSTIFY_RIGHT );
    } else if( screenShape != System.SCREEN_SHAPE_RECTANGLE || rectangleAlignment == Gfx.TEXT_JUSTIFY_CENTER ) {
  ...
  // Return the width for a line at certain Y position and with height
  ...
  function getWidthAt(posY) {
    // Descent G1 and Instinct 2 report wrong screenshape
